...
from config.settings import load_stock_pool, get_stock_name, DEFAULT_SIGNAL_SOURCE
...
def scan_all() -> list[dict]:
    ...
    for stock in stocks:
        ...
        try:
            ...
            today_signal = float(signal_df.iloc[-1][SOURCE_COLUMNS[source]])
            ...
            if today_decision != 0:
                ...
                alert = {
                    ...
                    "position": pos_label,
                    "source": source,
                    "signal": round(today_signal, 3),
                }
                alerts.append(alert)
                logger.info(f"  >>> {name} {action} 信号({pos_label})! 信号值={today_signal:.3f} 收盘价={today_close:.2f}")
            else:
...
def format_report(alerts: list[dict]) -> str:
    ...
    <table border="1" cellpadding="6" cellspacing="0" style="border-collapse:collapse">
        <tr style="background:#f0f0f0">
            <th>股票</th><th>操作</th><th>收盘价</th><th>信号源</th><th>信号值</th><th>仓位</th>
        </tr>
        {rows}
    </table>
    <br>
    <p style="color:#999;font-size:12px">
        信号值范围: -1(强卖)~+1(强买) | 缠论: B1=1.0 B2=0.7 B3=0.8 S1=-1.0 S2=-0.7 S3=-0.8
    </p>
    """
...
def run():
    ...
    for a in alerts:
        print(f"  {a['action']} | {a['name']}({a['code']}) | 收盘:{a['close']:.2f} "
              f"| {a['source']}信号:{a['signal']} | {a['position']}")
